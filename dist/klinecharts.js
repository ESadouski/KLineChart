...
function logTag() {
  {
    console.log('%c❤️ Welcome to klinecharts. Version is 8.3.5', 'border-radius:2px;border:dashed 1px #2196F3;padding:26px 20px;font-size:14px;color:#2196F3');
  }
}
  ...
  _createClass(ShapeStore, [{
    ...
    value: function setMouseOperate(mouseOperate) {
      ...
      if (mouseOperate.hover && (hover.id !== mouseOperate.hover.id || hover.element !== mouseOperate.hover.element || hover.elementIndex !== mouseOperate.hover.elementIndex)) {
        this._mouseOperate.hover = _objectSpread$6({}, mouseOperate.hover);
      }
      ...
      if (mouseOperate.click && (click.id !== mouseOperate.click.id || click.element !== mouseOperate.click.element || click.elementIndex !== mouseOperate.click.elementIndex)) {
        this._mouseOperate.click = _objectSpread$6({}, mouseOperate.click);
      }
    }
    /**
     * 是否为空
...
var AnnotationStore = /*#__PURE__*/function () {
  function AnnotationStore(chartStore) {
    ...
    this._visibleAnnotations = new Map(); // 注解鼠标操作信息
  ...
  _createClass(AnnotationStore, [{
    ...
    value: function setMouseOperate(operate) {
      ...
      if (operate && id !== operate.id) {
        this._mouseOperate = _objectSpread$5({}, operate);
      }
    }
    /**
     * 创建可见的注解数据
...
var ChartStore = /*#__PURE__*/function () {
  function ChartStore(styleOptions, handler) {
    ...
    this._dataList = [];
    this._asksBidsData = {}; // 可见的数据(需要绘制的数据)
  ...
  _createClass(ChartStore, [{
    ...
    key: "getAsksBidsData",
    value: function getAsksBidsData() {
    ...
    key: "addAsksBidsData",
    value: function addAsksBidsData(data) {
...
var YAxisOverlayView = /*#__PURE__*/function (_View) {
  ...
  function YAxisOverlayView(container, chartStore, yAxis, paneId) {
    ...
    _defineProperty(_assertThisInitialized(_this), "initialCoordinates", {
      asksChart: {
        x: 0,
        y: function y() {
          return _this._yAxis.convertToPixel(0)();
        }
      },
      bidsChart: {
        x: 0,
  ...
  _createClass(YAxisOverlayView, [{
    key: "_drawOrderChart",
    value: function _drawOrderChart(items, totalCount, chartType, styles) {
    ...
    key: "_drawLine",
    value: function _drawLine(orders, totalCount) {
      var _this2 = this;

      var currentX = 0;
      orders.forEach(function (order, index) {
        currentX += _this2._convertXToPixels(order.count, totalCount);

        _this2._ctx.lineTo(currentX, _this2._yAxis.convertToPixel(order.price));

        if (orders[index + 1]) {
          _this2._ctx.lineTo(currentX, _this2._yAxis.convertToPixel(orders[index + 1].price));
        }
      });
      return currentX;
    }
    ...
    key: "_drawOrderLine",
    value: function _drawOrderLine(orders, totalCount, chartType, styles) {
      this._ctx.beginPath();

      this._ctx.strokeStyle = styles.lineColor;
      this._ctx.lineWidth = 3;
      var initialCoords = chartType === 'asks' ? this.initialCoordinates.asksChart : this.initialCoordinates.bidsChart;
    ...
    key: "_drawOrdersArea",
    value: function _drawOrdersArea(orders, totalCount, chartType, styles) {
      this._ctx.beginPath();
      ...
      var currentX = this._drawLine(orders, totalCount);
    ...
    key: "_convertXToPixels",
    value: function _convertXToPixels(x, totalCount) {
      var fullWidth = this._yAxis.width();
    ...
    key: "_draw",
    value: function _draw() {
      var asksBidsData = this._chartStore.getAsksBidsData();

      if (asksBidsData.asks && asksBidsData.bids) {
        var styles = this._chartStore.styleOptions().yAxis;
...
var Chart = /*#__PURE__*/function () {
  ...
  _createClass(Chart, [{
    ...
    key: "applyAsksBidsData",
    value: function applyAsksBidsData(data) {
      var chartStore = this._chartPane.chartStore();
 ...
 * limitations under the License.
 */
extension.addShapeTemplate([horizontalRayLine, horizontalSegment, horizontalStraightLine, verticalRayLine, verticalSegment, verticalStraightLine, rayLine, segment, straightLine, parallelStraightLine, priceChannelLine, priceLine, fibonacciLine]);
